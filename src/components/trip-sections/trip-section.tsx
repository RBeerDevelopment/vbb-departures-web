...
export function TripSection(props: Props): React.ReactElement {
    ...
    return (
        <div className="flex flex-col items-start bg-white w-11/12 min-h-min rounded-md my-4 p-4">
            ...
            <div className="w-full flex flex-col items-center py-4">
                ...
                <div className="w-full flex justify-between items-center font-light text-sm px-5 py-2">
                    <p>{formatTime(trip.departure)} {Boolean(trip.departureDelay) && <DelayText delay={trip.departureDelay} />}</p>
                    <p>{formatTime(trip.arrival)} {Boolean(trip.arrivalDelay) && <DelayText delay={trip.arrivalDelay} />}</p>
                </div>
            </div>
        </div>
    );
