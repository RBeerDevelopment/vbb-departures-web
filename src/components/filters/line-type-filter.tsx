...
export function LineTypeFilter(props: Props): React.ReactElement {
    ...
    return (
        <select onChange={onChange} className="select select-bordered max-w-xs bg-white text-black">
            ...
            {lineTypes.sort(sortAlphabetically).map(l => <option key={l} value={l}>{LineTypeLabels[l]}</option>)}
        </select>
    );
}

function isLineTypeWithAll(value: unknown): value is LineTypeWithAll {
    return value === "All" || (typeof value === "string" && LineType.includes(value as LineType));
}