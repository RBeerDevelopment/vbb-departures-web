...
export function MapSection(props: Props): React.ReactElement {
  const { polylinePoints, lineName, currentLocation } = props;
  ...
  const currentLocationPolylinePoint = [
    currentLocation?.latitude || polylinePoints[0][0],
    currentLocation?.longitude || polylinePoints[0][1],
  ];

  const indexOfClosestPointOnLine = findIndexOfClosestPoint(
  ...
  const completedPolylinePoints = [
    ...polylinePoints.slice(0, indexOfClosestPointOnLine),
    currentLocationPolylinePoint,
  ];

  return (
    <div className="h-96 w-11/12 rounded-lg lg:w-1/2">
      <MapkitProvider tokenOrCallback={env.NEXT_PUBLIC_MAPKIT_TOKEN}>
        ...
        >
          ...
          <MapPolyline
            polylinePoints={polylinePoints}
            style={{ lineWidth: 3, strokeColor: lineColorCode }}
          />
          {/* section of the line that has been completed */}
          <MapPolyline
            polylinePoints={completedPolylinePoints}
            style={{ lineWidth: 8, strokeColor: lineColorCode }}
          />
